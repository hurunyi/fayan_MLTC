...
class F1Score(Metric):
    ...
    def __init__(self, thresh=0.5,
        ...
        assert task_type in ['binary', 'multiclass']
        assert average in ['binary', 'micro', 'macro', 'samples', 'weighted', 'both']
    ...
    def value(self):
        ...
        if self.average == "both":
            f1 = (f1_score(y_true=self.y_true, y_pred=self.y_pred, average="micro", zero_division=0) +
                  f1_score(y_true=self.y_true, y_pred=self.y_pred, average="macro", zero_division=0)) / 2
        else:
